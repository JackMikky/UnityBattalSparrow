...
public class LightFlicker : MonoBehaviour//¥é¥¤¥Ö¥é¥E`¡¡EÇò¥³¥ó¥È¥úÅ`¥é©`
{
    ...
    //const string k_EmissiveColorName = "Emission";
    ...
    void Start()
    {
        ...
        m_FlickeringMaterial.EnableKeyword(k_EmissiveColorName);
        //  m_EmissionColor = m_FlickeringMaterial.GetColor(k_EmissionColorID);
        if (m_FlickeringMaterial.HasColor(k_EmissionColorID))
        {
            m_EmissionColor = m_FlickeringMaterial.GetColor(k_EmissionColorID);
        }
        else
        {
            m_EmissionColor = m_FlickeringMaterial.GetColor("_EMISSION_COLOR");
        }
    }
    ...
    void Update()
    {
        ...
        if (m_EmissionColor != null)
        {
            m_FlickeringMaterial.SetColor(k_EmissionColorID, m_EmissionColor * m_FlickerLightIntensity * k_LightIntensityToEmission);
            //for (var i = 0; i < flickeringRenderer.Length; i++)
            //{
            //    flickeringRenderer[i].material.SetColor(k_EmissionColorID, m_EmissionColor * m_FlickerLightIntensity * k_LightIntensityToEmission);
            //}
        }
